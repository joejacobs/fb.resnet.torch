...
function main()
    ...
    local data = Utils.image_to_th( Utils.data_to_th( npy4th.loadnpy(
        string.format('%s/PreComp/Features/%s.npy',opt.data_dir,opt.inputs)
        ):double() ) )
    ...
    -- transform data to ImageNet format
    local meanstd = {
    ...
    local tmp_data = torch.Tensor( data:size(1), data:size(2), 224, 224 )
    ...
    local fst,inputs,lst,nn_features,tmp
